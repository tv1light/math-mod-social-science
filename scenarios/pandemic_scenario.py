...
def pandemic_scenario():
    # Параметры нормального сценария (до пандемии)
    normal_params = (1.085e-5, 6.51e-12, 8.2e-6)
    catastrophe_params = (normal_params[0] // 3, normal_params[1] // 3, normal_params[2] // 3)  # Измененные параметры

    # Начальные условия
    N0, S0, l0 = 170000000, 17.47, 0.052
    initial_conditions = [N0, S0, l0]

    # Разделение временного интервала на две части: до и после пандемии
    t_split = 2025   # Год наступления пандемии
    ...
    final_conditions_pre = [sol_pre.y[0, -1], sol_pre.y[1, -1], sol_pre.y[2, -1]]

    # Решение после пандемии
